...
const ChiefAllReports = () => {
	...
	useEffect(() => {
		const fetchAllReports = async () => {
			try {
				...
				if (!userPos) {
					setError('Pos user tidak ditemukan');
					return;
				}

				const response = await axiosInstance.get(`/api/logbook-harian-master-chief/${userPos}`);
				
				console.log('API Response:', response);
				console.log('Response data:', response.data);
				
				// Handle backend response structure: { total, lokasi, pos, laporan: [...] }
				let reportsArray = [];
				
				if (response.data && response.data.laporan && Array.isArray(response.data.laporan)) {
					// Backend returns { laporan: [...] }
					reportsArray = response.data.laporan;
					console.log('Using laporan array from response:', reportsArray.length, 'reports');
					
					// Store additional response info
					setResponseInfo({
						total: response.data.total || reportsArray.length,
						lokasi: response.data.lokasi || userPos,
						pos: response.data.pos || userPos
					});
				} else if (response.data && Array.isArray(response.data)) {
					// Direct array response (fallback)
					reportsArray = response.data;
					console.log('Using direct array response:', reportsArray.length, 'reports');
					
					// Set default info for direct array response
					setResponseInfo({
					...
					setError('Struktur data tidak valid');
					setReports([]);
					return;
				}

				// Add type and modelName for navigation
				const reportsWithType = reportsArray.map(report => ({
					...report,
					type: 'Logbook Harian',
					modelName: 'logbook_harian_master'
				}));

				console.log('Processed reports:', reportsWithType.length);
		...
		if (auth?.user?.pos) fetchAllReports();
	}, [auth?.user?.pos]);
	...
	const handleReportClick = (report) => {
		if (report.status === 'Completed' || report.status === 'Submitted') {
			navigate(`/logbook-preview/${report.id}`);
		} else {
			navigate(`/forms/masters/logbook-chief/${report.id}`);
		}
	};
	...
	const handleCreate = () => {
		const userPos = auth?.user?.pos;
		if (userPos) {
			navigate(`/forms/masters/logbook-chief/create?lokasi=${encodeURIComponent(userPos)}`);
		} else {
				...
				<div className="mb-6">
					<div className="flex items-center justify-between mb-4">
						...
						<div className="flex space-x-3">
							<button onClick={() => navigate('/dashboard/chief')} className="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition-colors">
								Kembali ke Dashboard
							</button>
						</div>
					</div>
					<p className="text-gray-600">
						Menampilkan semua laporan di Unit: <span className="font-semibold text-blue-600">{responseInfo.lokasi || auth?.user?.pos || 'Tidak diketahui'}</span>
						{responseInfo.total > 0 && (
							<span className="ml-2 text-gray-500">• Total: {responseInfo.total} laporan</span>
						)}
					</p>
				</div>
								...
								{currentReports.length === 0 ? (
									<tr>
										<td colSpan="5" className="px-6 py-4 text-center text-gray-500">
											{reports.length === 0 ? 'Tidak ada laporan di lokasi ini' : 'Tidak ada laporan yang sesuai dengan filter'}
										</td>
									</tr>
								) : (
										...
										<tr key={`${report.modelName}-${report.id}`} className="hover:bg-gray-50 cursor-pointer" onClick={() => handleReportClick(report)}>
											...
											<td className="px-6 py-4 whitespace-nowrap">
												<button className="text-blue-600 hover:text-blue-900 font-medium">
													{report.status === 'Completed' || report.status === 'Submitted' ? 'Lihat Preview' : 'Edit'}
												</button>
											</td>
										</tr>
									))
				...
				<div className="mt-6 bg-white rounded-lg shadow p-4">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
						<div>
							...
							<p className="text-2xl font-bold text-blue-600">{responseInfo.total || reports.length}</p>
						</div>
						<div>
							<p className="text-sm text-gray-600">Pos</p>
							<p className="text-lg font-semibold text-gray-800">{responseInfo.lokasi || auth?.user?.pos || 'Tidak diketahui'}</p>
						</div>
						<div>
							<p className="text-sm text-gray-600">Pos</p>
							<p className="text-lg font-semibold text-gray-800">{responseInfo.pos || auth?.user?.pos || 'Tidak diketahui'}</p>
						</div>
					</div>
				</div>
			</div>
...
export default ChiefAllReports; 