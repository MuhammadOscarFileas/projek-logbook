...
const LogbookHarianMasterTable = () => {
  ...
  const handleCardClick = (logbook) => {
    if (logbook.status === 'Completed' || logbook.status === 'Submitted') {
      // For completed logbooks, go to preview
      navigate(`/logbook-preview/${logbook.id}`);
    } else {
      // For draft and submitted, go to edit form
      navigate(`/forms/masters/logbook-harian/${logbook.id}`);
    }
  };
  ...
  // Pagination for submitted/completed reports
        ...
        <div className="space-y-8">
          ...
          <div>
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-xl font-semibold text-gray-900">Draft ({draftLogbooks.length})</h3>
            </div>
            {draftLogbooks.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                Tidak ada logbook dalam status Draft
              </div>
            ) : (
          ...
          {/* Submitted & Completed Section - Combined as table */}
          <div>
            <div className="flex items-center mb-4">
            ...
            {submittedCompletedLogbooks.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                Tidak ada logbook dalam status Submitted atau Completed
              </div>
            ) : (
              <>
                {/* Table for Submitted & Completed */}
                          ...
                          <tr key={logbook.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => handleCardClick(logbook)}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                              <div className="flex space-x-2">
                                <button
                                  onClick={(e) => handlePreview(e, logbook.id)}
